(() => {
  ...
  const META = {
    quests:['Quêtes','Tes priorités du jour et tes quêtes secondaires, toujours à portée de main.'],
  ...
  const todayDrawerGrip = document.querySelector('#todayDrawerGrip');
  const todayDrawerDate = document.querySelector('#todayDrawerDate');
  const todayQuestList = document.querySelector('#todayQuestList');
  const todayQuestAdd = document.querySelector('#todayQuestAdd');
  const tabs = [...document.querySelectorAll('[data-journal-tab]')];
  if (!dialog || !openButton || !closeButton || !content || !todayDrawer || !todayDrawerGrip || !todayQuestList || !todayQuestAdd || !tabs.length) return;
  ...
  const fields = {
    ...
    today:document.querySelector('#journalToday'), total:document.querySelector('#journalTotal'),
    coins:document.querySelector('#journalCoins')
  };
  ...
  let timerId = 0, saveTimer = 0, diaryInkTimer = 0, modalCloseTimer = 0, modalReturnFocus = null;
  let drawerOpen = true, drawerPointer = null, sceneSwipe = null, drawerSuppressClick = false, diaryAnimateDate = '';
  ...
  function normalize(raw){
    ...
    s.quests.forEach(item=>{
      ...
      if(item.type!=='evenement'){
        ...
        if(item.type==='secondaire'){item.rec='tous';item.days=[0,1,2,3,4,5,6];item.date='';item.time='';item.fin='';item.dur=0;item.steps=[];item.abandonedToday=false;item.abandonedDay='';}
      }
    }); return s;
  }
  ...
  function rollover(){
    ...
    Object.keys(state.diary.days).forEach(date=>{const entry=state.diary.days[date];if(!stripGeneratedQuestion(entry.t,date).trim())delete state.diary.days[date];});
  ...
  function activeOn(item,date){if(item.type==='evenement')return item.date===date;if(item.type==='secondaire')return true;if(item.rec==='oneshot')return(item.date||today())===date;return(item.days||[]).includes(dow(fromIso(date)));}
  function recurrence(item){if(item.type==='secondaire')return'Quand tu veux';if(item.rec==='oneshot')return item.date===today()?'Aujourd’hui':formatDate(item.date||today(),{day:'numeric',month:'short'});const days=item.days||[];return days.length===7?'Tous les jours':days.map(d=>DAYS[d]).join(' · ');}
  ...
  function updateProfile(){
    ...
    fields.today.textContent=completed; fields.total.textContent=state.totalDone; fields.coins.textContent=state.coins;
  }
  ...
  function setDrawerOpen(open){
    drawerOpen=Boolean(open);todayDrawer.classList.toggle('is-open',drawerOpen);todayDrawer.classList.toggle('is-closed',!drawerOpen);todayDrawer.classList.remove('is-dragging');todayDrawer.style.removeProperty('transform');todayDrawerGrip.setAttribute('aria-expanded',String(drawerOpen));todayDrawerGrip.setAttribute('aria-label',drawerOpen?'Masquer les quêtes du jour':'Afficher les quêtes du jour');
  }
  function renderTodayDrawer(){
    todayDrawerDate.textContent='Glisser vers le bas pour voir';
    todayQuestList.replaceChildren();
    const items=state.quests.filter(item=>item.type!=='evenement'&&activeOn(item,today())&&!doneToday(item)&&!abandonedToday(item)).sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99')||(a.title||'').localeCompare(b.title||'','fr'));
    if(!items.length){
      const emptyState=node('div','today-quest-empty today-quest-victory'),wreath=document.createElementNS('http://www.w3.org/2000/svg','svg');
      wreath.classList.add('today-victory-wreath');wreath.setAttribute('viewBox','0 0 220 150');wreath.setAttribute('role','img');wreath.setAttribute('aria-label','Couronne de lauriers, toutes les quêtes sont accomplies');
      const laurel='<path class="laurel-stem" d="M104 140C73 132 49 114 35 89C23 68 23 44 34 20"/><ellipse class="laurel-leaf" cx="88" cy="130" rx="18" ry="6.5" transform="rotate(18 88 130)"/><ellipse class="laurel-leaf" cx="68" cy="116" rx="18" ry="6.5" transform="rotate(30 68 116)"/><ellipse class="laurel-leaf" cx="51" cy="96" rx="18" ry="6.5" transform="rotate(42 51 96)"/><ellipse class="laurel-leaf" cx="39" cy="74" rx="18" ry="6.5" transform="rotate(60 39 74)"/><ellipse class="laurel-leaf" cx="34" cy="49" rx="17" ry="6.3" transform="rotate(78 34 49)"/><ellipse class="laurel-leaf" cx="43" cy="38" rx="17" ry="6.3" transform="rotate(-48 43 38)"/><ellipse class="laurel-leaf" cx="49" cy="61" rx="17" ry="6.3" transform="rotate(-30 49 61)"/><ellipse class="laurel-leaf" cx="58" cy="84" rx="17" ry="6.3" transform="rotate(-20 58 84)"/><ellipse class="laurel-leaf" cx="74" cy="104" rx="17" ry="6.3" transform="rotate(-8 74 104)"/>';
      wreath.innerHTML=`<g>${laurel}</g><g transform="translate(220 0) scale(-1 1)">${laurel}</g>`;
      emptyState.append(wreath,node('p','','Toutes les quêtes du jour sont accomplies.'));todayQuestList.append(emptyState);return;
    }
    items.forEach(item=>{
      const card=node('article','today-quest-card'),icon=node('span','today-quest-icon',item.icon||'⚔️'),copy=node('div','today-quest-copy'),time=node('time','',item.time||'À tout moment'),title=node('strong','',item.title||'Sans titre'),validate=node('button','today-quest-validate','');
      validate.type='button';validate.addEventListener('click',()=>complete(item));
      copy.append(time,title);validate.setAttribute('aria-label',`Valider ${item.title}`);card.append(icon,copy,validate);todayQuestList.append(card);
    });
  }
  function beginDrawerGesture(event){
    if(event.button!==undefined&&event.button!==0)return;const height=todayDrawer.getBoundingClientRect().height,gripHeight=todayDrawerGrip.getBoundingClientRect().height;drawerPointer={id:event.pointerId,startY:event.clientY,lastY:event.clientY,startOffset:drawerOpen?0:-(height-gripHeight),minOffset:-(height-gripHeight),moved:false};todayDrawer.classList.add('is-dragging');todayDrawerGrip.setPointerCapture?.(event.pointerId);
  }
  function moveDrawerGesture(event){
    if(!drawerPointer||event.pointerId!==drawerPointer.id)return;const delta=event.clientY-drawerPointer.startY,offset=Math.max(drawerPointer.minOffset,Math.min(0,drawerPointer.startOffset+delta));drawerPointer.lastY=event.clientY;drawerPointer.moved=drawerPointer.moved||Math.abs(delta)>8;todayDrawer.style.transform=`translate(-50%, ${offset}px)`;event.preventDefault();
  }
  ...
  function renderQuestCard(item,parent){
    const completed=doneToday(item),reward=gainFor(item),card=node('article',`journal-quest-card ${item.type}${completed?' is-complete':''}`),icon=node('span','journal-card-icon',item.icon||'⚔️'),body=node('div','journal-card-body'),line=node('div','journal-card-titleline');
    ...
    if(selectedDate===today()){
      if(item.type==='secondaire'&&completed){const doneButton=button('✓ Validée aujourd’hui','secondary');doneButton.disabled=true;actions.append(doneButton);}
      else{if(item.steps?.length)actions.append(button(`☑ Étapes ${item.steps.filter(s=>s.d).length}/${item.steps.length}`,'secondary',()=>openSteps(item)));actions.append(button(item.dur>0?'▶ Accepter':'✓ Valider','primary',()=>item.dur>0?openTimer(item):complete(item)));}
    }
    ...
    if(item.type!=='secondaire'&&selectedDate===today()&&(item.time||item.fin))options.append(button(item.snoozeDay===today()?'↩ Reprendre':'⏰ Repousser','secondary',()=>{item.snoozeDay=item.snoozeDay===today()?'':today();saveAndRender();toast(item.snoozeDay?'Quête repoussée à plus tard.':'Quête reprise.');}));
    if(item.type!=='secondaire'&&selectedDate===today())options.append(button('✕ Refuser','danger',()=>abandon(item)));options.append(button('✎ Modifier','secondary',()=>openQuestForm(item)),button('🗑 Retirer','ghost',()=>removeQuest(item)));
    body.append(line,chips,actions,options);card.append(icon,body);parent.append(card);
  }
  function renderQuests(root){
    const paths=node('div','journal-paths');paths.setAttribute('aria-label','Types de quêtes');[['principale','⚔️','Principales'],['secondaire','✦','Secondaires']].forEach(([type,icon,label])=>{const count=state.quests.filter(x=>x.type===type&&activeOn(x,selectedDate)&&(type==='secondaire'||(!doneToday(x)&&!abandonedToday(x)))).length,b=button(`${icon} ${label}${count?` · ${count}`:''}`,questPath===type?'is-active':'',()=>{questPath=type;render();});b.setAttribute('aria-pressed',String(questPath===type));paths.append(b);});root.append(paths);renderDayStrip(root);
    root.append(node('div','journal-date-title',selectedDate===today()?'Aujourd’hui':formatDate(selectedDate)));
    const goal=node('div','journal-goal'),count=completedToday();goal.innerHTML=`<span><b>◇</b> Objectif du jour</span><strong>${Math.min(count,state.objJour)} / ${state.objJour}</strong><i style="--goal:${Math.min(100,count/state.objJour*100)}%"></i>`;root.append(goal);rule(root,questPath==='principale'?'Chemin principal':'Quêtes secondaires');
    const list=node('div','journal-list'),items=state.quests.filter(x=>x.type===questPath&&activeOn(x,selectedDate)&&(questPath==='secondaire'||!(selectedDate===today()&&(doneToday(x)||abandonedToday(x))))).sort((a,b)=>Number(a.snoozeDay===today())-Number(b.snoozeDay===today())||(a.time||'99').localeCompare(b.time||'99'));
    if(!items.length)empty(list,questPath==='principale'?'⚔️':'✨',questPath==='principale'?'Aucune quête principale pour cette journée.':'Aucune quête secondaire pour le moment. Ajoute une petite activité à faire quand tu en as envie.');items.forEach(x=>renderQuestCard(x,list));root.append(list);
  }
  ...
  function renderRoutine(root){
    const toggle=button(`🗓️ Routine de la semaine ${weekOpen?'⌄':'›'}`,'journal-routine-toggle',()=>{const top=toggle.getBoundingClientRect().top;weekOpen=!weekOpen;render();const replacement=content.querySelector('.journal-routine-toggle');if(replacement)content.scrollTop+=replacement.getBoundingClientRect().top-top;});toggle.setAttribute('aria-expanded',String(weekOpen));root.append(toggle);if(!weekOpen)return;
    const block=node('div','journal-routine'),start=monday(new Date());
    for(let i=0;i<7;i++){const date=new Date(start);date.setDate(start.getDate()+i);const key=iso(date),day=node('section','journal-routine-day');day.append(node('h3','',`${DAYS[i]} ${date.getDate()}`));const rows=state.quests.filter(x=>x.type==='principale'&&x.rec!=='oneshot'&&activeOn(x,key)).sort((a,b)=>(a.time||'99').localeCompare(b.time||'99'));if(!rows.length)day.append(node('p','','Repos'));rows.forEach(x=>day.append(button(`${x.time?`${x.time} · `:''}${x.icon} ${x.title}`,`journal-routine-row ${x.type}`,()=>openQuestForm(x))));block.append(day);}block.append(button('+ Nouvelle routine','primary wide',()=>openQuestForm(null,'principale')));root.append(block);
  }
  ...
  function stripGeneratedQuestion(text,date){const value=String(text||''),generated=`✨ ${dailyQuestion(date)}\n`;return value.startsWith(generated)?value.slice(generated.length).replace(/^\n/,''):value;}
  ...
  function cleanupDiaryDay(date){const entry=state.diary.days[date];if(!entry)return false;if(stripGeneratedQuestion(entry.t,date).trim())return false;delete state.diary.days[date];return true;}
  function changeDiaryDate(nextDate){clearTimeout(saveTimer);clearTimeout(diaryInkTimer);diaryAnimateDate='';cleanupDiaryDay(diaryDate);save();diaryDate=nextDate;render();}
  function acceptDiaryQuestion(entry,date){
    const generated=`✨ ${dailyQuestion(date)}\n\n`;entry.qa=1;entry.t=generated+stripGeneratedQuestion(entry.t,date);diaryAnimateDate=date;saveAndRender();
  }
  function animateDiaryQuestion(textarea,entry,date){
    clearTimeout(diaryInkTimer);const full=entry.t||'',reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;if(reduced||!full){textarea.value=full;return;}
    textarea.value='';textarea.readOnly=true;textarea.classList.add('is-inking');let index=0;
    const write=()=>{if(!textarea.isConnected||diaryDate!==date){clearTimeout(diaryInkTimer);return;}index=Math.min(full.length,index+1);textarea.value=full.slice(0,index);textarea.scrollTop=0;if(index<full.length){const char=full[index-1],delay=/[?.!]/.test(char)?145:/[,;:]/.test(char)?90:38;diaryInkTimer=setTimeout(write,delay);}else{textarea.readOnly=false;textarea.classList.remove('is-inking');textarea.setSelectionRange(full.length,full.length);}};
    diaryInkTimer=setTimeout(write,180);
  }
  function renderDiary(root){
    const entry=getDiaryDay(diaryDate),nav=node('div','journal-diary-nav');nav.append(button('‹','round',()=>changeDiaryDate(addDays(diaryDate,-1))),node('strong','',formatDate(diaryDate)),button('›','round',()=>changeDiaryDate(addDays(diaryDate,1))));root.append(nav);
    if(diaryDate===today()&&!entry.qa&&!entry.qRef){const prompt=node('div','journal-diary-prompt'),question=node('span','');question.append(node('b','','✦'),document.createTextNode(` ${dailyQuestion(diaryDate)}`));const choices=node('div');choices.append(button('Accepter','primary',()=>acceptDiaryQuestion(entry,diaryDate)),button('Refuser','ghost',()=>{entry.qRef=1;saveAndRender();}));prompt.append(question,choices);root.append(prompt);}
    const page=node('div','journal-diary-page'),textarea=node('textarea','journal-diary-text');textarea.value=entry.t||'';textarea.placeholder=entry.qa?'Écris ta réponse ici…':'Écris ici — pensées, humeur, victoires…';textarea.setAttribute('aria-label','Page du carnet');textarea.setAttribute('aria-live','polite');textarea.oninput=()=>{entry.t=textarea.value;clearTimeout(saveTimer);saveTimer=setTimeout(save,350);};const writtenPages=Object.entries(state.diary.days).filter(([date,day])=>stripGeneratedQuestion(day?.t,date).trim()).length;page.append(textarea,node('footer','',writtenPages?`${writtenPages} page${writtenPages>1?'s':''} conservée${writtenPages>1?'s':''}`:'Cette page vide ne sera pas conservée'));root.append(page);if(diaryAnimateDate===diaryDate){diaryAnimateDate='';animateDiaryQuestion(textarea,entry,diaryDate);}
  }

  function render(options={}){
    rollover();updateProfile();const root=createPanel(options);if(activeTab==='quests')renderQuests(root);if(activeTab==='agenda')renderAgenda(root);if(activeTab==='history')renderHistory(root);if(activeTab==='diary')renderDiary(root);
  ...
  function selectTab(tab){if(!META[tab])return;const changed=activeTab!==tab;if(changed&&activeTab==='diary'){clearTimeout(saveTimer);clearTimeout(diaryInkTimer);diaryAnimateDate='';cleanupDiaryDay(diaryDate);save();}activeTab=tab;render({resetScroll:changed,animate:changed});}
  function saveAndRender(){save();render();renderTodayDrawer();}
  ...
  function openSecondaryForm(editing=null,draftTitle=''){
    let icon=editing?.icon||'✨';
    const box=openModal(editing?'Modifier la quête secondaire':'Nouvelle quête secondaire','Une petite activité toujours disponible, à valider une fois par jour.');
    box.innerHTML+=`<form class="journal-form journal-bonus-form" id="bonusForm"><label>Titre<input name="title" maxlength="42" required value="${esc(draftTitle||editing?.title||'')}" placeholder="Ex : Lire quelques pages"></label><fieldset><legend>Émoticône</legend><div class="journal-icon-grid">${ICONS.map(value=>`<button type="button" data-icon="${value}">${value}</button>`).join('')}</div></fieldset><p class="journal-bonus-note"><b>Quête secondaire quotidienne</b><span>Elle reste dans ta liste chaque jour et redevient disponible le lendemain après validation.</span></p><div class="journal-reward-preview"></div><div class="journal-modal-actions"><button class="journal-button ghost" type="button" data-cancel>Annuler</button><button class="journal-button primary" type="submit">${editing?'Enregistrer':'Créer'}</button></div>${editing?'<button class="journal-button danger wide" type="button" data-delete>Supprimer la quête</button>':''}</form>`;
    const form=box.querySelector('#bonusForm'),draw=()=>{form.querySelectorAll('[data-icon]').forEach(n=>{const on=n.dataset.icon===icon;n.classList.toggle('is-active',on);n.setAttribute('aria-pressed',String(on));});const reward=gainFor({type:'secondaire'});form.querySelector('.journal-reward-preview').textContent=`Récompense : +${reward.xp} XP · +${reward.gold} or`;};
    form.querySelector('.journal-icon-grid').onclick=e=>{const choice=e.target.closest('[data-icon]');if(choice){icon=choice.dataset.icon;draw();}};
    form.querySelector('[data-cancel]').onclick=closeModal;form.querySelector('[data-delete]')?.addEventListener('click',()=>removeQuest(editing));
    form.onsubmit=e=>{e.preventDefault();const values=new FormData(form),result={id:editing?.id||uid(),title:String(values.get('title')||'').trim()||'Quête sans nom',type:'secondaire',icon,rec:'tous',date:'',days:[0,1,2,3,4,5,6],time:'',fin:'',dur:0,steps:[],doneToday:editing?.doneToday||false,doneDay:editing?.doneDay||'',abandonedToday:false,abandonedDay:'',snoozeDay:'',creeT:editing?.creeT||Date.now()};if(editing)Object.assign(editing,result);else state.quests.push(result);questPath='secondaire';selectedDate=today();closeModal();saveAndRender();toast(editing?'Quête secondaire modifiée.':'Quête secondaire ajoutée.');};
    draw();
  }
  function openQuestForm(editing=null,preferredType=questPath){
    if(editing?.type==='secondaire'||(!editing&&preferredType==='secondaire')){openSecondaryForm(editing);return;}
    ...
    typeChoice.onclick=e=>{const choice=e.target.closest('[data-value]');if(!choice)return;if(choice.dataset.value==='secondaire'){const draft=form.elements.title.value;closeModal();openSecondaryForm(editing,draft);return;}type=choice.dataset.value;update();};recChoice.onclick=e=>{if(e.target.dataset.value){rec=e.target.dataset.value;update();}};form.querySelector('.journal-day-choices').onclick=e=>{if(e.target.dataset.day!=null){const day=Number(e.target.dataset.day);days=days.includes(day)?days.filter(v=>v!==day):[...days,day];update();}};form.querySelector('.journal-icon-grid').onclick=e=>{if(e.target.dataset.icon){icon=e.target.dataset.icon;update();}};['hasTime','hasSteps','time','fin'].forEach(name=>form.elements[name].addEventListener('input',update));
  ...
  function openModal(title,subtitle=''){
    clearInterval(timerId);clearTimeout(modalCloseTimer);if(modalLayer.hidden)modalReturnFocus=document.activeElement;
    modalLayer.hidden=false;modalLayer.classList.remove('is-open','is-closing');modalLayer.replaceChildren();
    const sheet=node('section','journal-modal'),head=node('header','journal-modal-head'),copy=node('div'),heading=node('h2','',title);heading.id='journalModalTitle';sheet.setAttribute('role','dialog');sheet.setAttribute('aria-modal','true');sheet.setAttribute('aria-labelledby',heading.id);copy.append(heading);if(subtitle)copy.append(node('p','',subtitle));const close=button('×','journal-modal-close',closeModal);close.setAttribute('aria-label','Fermer');head.append(copy,close);sheet.append(head);modalLayer.append(sheet);requestAnimationFrame(()=>{modalLayer.classList.add('is-open');modalLayer.querySelector('.journal-modal-close')?.focus({preventScroll:true});});return sheet;
  }
  function closeModal(){
    clearInterval(timerId);clearTimeout(modalCloseTimer);if(modalLayer.hidden)return;modalLayer.classList.remove('is-open');modalLayer.classList.add('is-closing');
    const finish=()=>{modalLayer.hidden=true;modalLayer.classList.remove('is-closing');modalLayer.replaceChildren();const target=modalReturnFocus;modalReturnFocus=null;requestAnimationFrame(()=>{if(target?.isConnected&&!target.disabled)target.focus({preventScroll:true});else if(dialog.open)fab.focus({preventScroll:true});});};
    if(matchMedia('(prefers-reduced-motion: reduce)').matches)finish();else modalCloseTimer=setTimeout(finish,160);
  }
  ...
  function checkSchedule(){
    ...
    state.quests.filter(x=>x.type==='principale'&&activeOn(x,today())&&!doneToday(x)&&!abandonedToday(x)).forEach(item=>{
    ...
    if(changed){save();renderTodayDrawer();if(dialog.open&&modalLayer.hidden)render();}
  }
  ...
  function openJournal(){rollover();render({resetScroll:true,animate:true});todayDrawer.hidden=true;dialog.showModal();openButton.hidden=true;}
  function closeJournal(){if(activeTab==='diary'){clearTimeout(saveTimer);clearTimeout(diaryInkTimer);diaryAnimateDate='';cleanupDiaryDay(diaryDate);save();}closeModal();dialog.close();todayDrawer.hidden=false;openButton.hidden=false;openButton.focus({preventScroll:true});renderTodayDrawer();}

  openButton.onclick=openJournal;closeButton.onclick=closeJournal;todayQuestAdd.onclick=openQuestFromDrawer;todayDrawerGrip.addEventListener('pointerdown',beginDrawerGesture);todayDrawerGrip.addEventListener('pointermove',moveDrawerGesture);todayDrawerGrip.addEventListener('pointerup',endDrawerGesture);todayDrawerGrip.addEventListener('pointercancel',endDrawerGesture);todayDrawerGrip.addEventListener('click',()=>{if(drawerSuppressClick)return;setDrawerOpen(!drawerOpen);});document.addEventListener('pointerdown',beginSceneSwipe,{passive:true});document.addEventListener('pointermove',moveSceneSwipe,{passive:false});document.addEventListener('pointerup',endSceneSwipe,{passive:true});document.addEventListener('pointercancel',()=>{sceneSwipe=null;},{passive:true});tabs.forEach(tab=>tab.onclick=()=>selectTab(tab.dataset.journalTab));fab.onclick=()=>activeTab==='agenda'?openEventForm():openQuestForm(null,questPath);modalLayer.onclick=e=>{if(e.target===modalLayer||e.target.closest('.journal-modal-close'))closeModal();};modalLayer.addEventListener('keydown',e=>{if(modalLayer.hidden)return;if(e.key==='Escape'){e.preventDefault();closeModal();return;}if(e.key!=='Tab')return;const focusable=[...modalLayer.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')].filter(element=>element.getClientRects().length);if(!focusable.length){e.preventDefault();return;}const first=focusable[0],last=focusable.at(-1);if(e.shiftKey&&(document.activeElement===first||!modalLayer.contains(document.activeElement))){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}});dialog.addEventListener('cancel',e=>{e.preventDefault();if(!modalLayer.hidden)closeModal();else closeJournal();});window.addEventListener('storage',e=>{if(e.key===KEY){state=load();renderTodayDrawer();if(dialog.open)render();}});document.addEventListener('visibilitychange',()=>{if(!document.hidden){rollover();checkSchedule();renderTodayDrawer();if(dialog.open)render();}});setInterval(checkSchedule,30000);render();renderTodayDrawer();setDrawerOpen(true);if(params.get('journal')==='1')requestAnimationFrame(openJournal);
})();
